...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie06_CzyWszyscyProwadzacyMajaKatedre()
    {
        var result = DaneUczelni.Prowadzacy
            .All(p => string.IsNullOrEmpty(p.Katedra.Trim()));
        return [result ? "0" : "1"];
    }
    ...
    {
        var reult = DaneUczelni.Studenci
    ...
    {
        var result = DaneUczelni.Zapisy
            .Join(DaneUczelni.Studenci, z => z.StudentId, s => s.Id, (z, s) => new { z.PrzedmiotId, s })
            .Join(DaneUczelni.Przedmioty, x => x.PrzedmiotId, p => p.Id, (x, p) => $"{x.s.Imie} {x.s.Nazwisko}, {p.Nazwa}");
        return result;
    }
    ...
    {
        var result = DaneUczelni.Prowadzacy
            .GroupJoin(DaneUczelni.Przedmioty, pr => pr.Id, p => p.ProwadzacyId, (pr, p) => $"{pr.Imie} {pr.Nazwisko}, {p.Count()}");
        return result;
    }
    ...
    public IEnumerable<string> Zadanie16_NajwyzszaOcenaKazdegoStudenta()
    {
        var result = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy
                .Where(z => z.OcenaKoncowa != null), s => s.Id, z => z.StudentId, (s, z) => new { s.Imie, s.Nazwisko, z.OcenaKoncowa })
            .GroupBy(x => new { x.Imie, x.Nazwisko })
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko} - {g.Max(g => g.OcenaKoncowa)}");
        return result;
    }
    ...
    public IEnumerable<string> Wyzwanie01_StudenciZWiecejNizJednymAktywnymPrzedmiotem()
    {
        var result = DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy
                .Where(z => z.CzyAktywny), s => s.Id, z => z.StudentId, (s, z) => s)
    ...
    {
        var result = DaneUczelni.Prowadzacy
           .Join(DaneUczelni.Przedmioty, pr => pr.Id, p => p.ProwadzacyId, (pr, p) => new { pr, p.Id })
           .Join(DaneUczelni.Zapisy
                .Where(z => z.OcenaKoncowa != null), x => x.Id, z => z.PrzedmiotId, (x, z) => new { x.pr.Imie, x.pr.Nazwisko, z.OcenaKoncowa })
           .GroupBy(x => new { x.Imie, x.Nazwisko })
